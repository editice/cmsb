package com.hj.dal.domain.dataobject;

/**
 * @author tinglang (editice@gmail.com)
 * @since 2015/2/3  15:49
 */
public class MenetYibaoDO {
    long id;
    String ROWKEY;
    String _xx;
    String ypmc;
    String _qy;
    String lb;
    String ybfl;
    String jx;
    String zldl;
    String zlzl;
    String detailUrl;
    String qy;
    String _jx;
    String _xzsyfw;
    String _ypmc;
    String _dl;
    String _jyfl;
    String ypmc_en;
    String _yl;
    String _ypfl;
    String zlxl;
    String f21;

    public long getId() {
        return id;
    }

    public void setId(long id) {
        this.id = id;
    }

    public String getROWKEY() {
        return ROWKEY;
    }

    public void setROWKEY(String ROWKEY) {
        this.ROWKEY = ROWKEY;
    }

    public String get_xx() {
        return _xx;
    }

    public void set_xx(String _xx) {
        this._xx = _xx;
    }

    public String getYpmc() {
        return ypmc;
    }

    public void setYpmc(String ypmc) {
        this.ypmc = ypmc;
    }

    public String get_qy() {
        return _qy;
    }

    public void set_qy(String _qy) {
        this._qy = _qy;
    }

    public String getLb() {
        return lb;
    }

    public void setLb(String lb) {
        this.lb = lb;
    }

    public String getYbfl() {
        return ybfl;
    }

    public void setYbfl(String ybfl) {
        this.ybfl = ybfl;
    }

    public String getJx() {
        return jx;
    }

    public void setJx(String jx) {
        this.jx = jx;
    }

    public String getZldl() {
        return zldl;
    }

    public void setZldl(String zldl) {
        this.zldl = zldl;
    }

    public String getZlzl() {
        return zlzl;
    }

    public void setZlzl(String zlzl) {
        this.zlzl = zlzl;
    }

    public String getDetailUrl() {
        return detailUrl;
    }

    public void setDetailUrl(String detailUrl) {
        this.detailUrl = detailUrl;
    }

    public String getQy() {
        return qy;
    }

    public void setQy(String qy) {
        this.qy = qy;
    }

    public String get_jx() {
        return _jx;
    }

    public void set_jx(String _jx) {
        this._jx = _jx;
    }

    public String get_xzsyfw() {
        return _xzsyfw;
    }

    public void set_xzsyfw(String _xzsyfw) {
        this._xzsyfw = _xzsyfw;
    }

    public String get_ypmc() {
        return _ypmc;
    }

    public void set_ypmc(String _ypmc) {
        this._ypmc = _ypmc;
    }

    public String get_dl() {
        return _dl;
    }

    public void set_dl(String _dl) {
        this._dl = _dl;
    }

    public String get_jyfl() {
        return _jyfl;
    }

    public void set_jyfl(String _jyfl) {
        this._jyfl = _jyfl;
    }

    public String getYpmc_en() {
        return ypmc_en;
    }

    public void setYpmc_en(String ypmc_en) {
        this.ypmc_en = ypmc_en;
    }

    public String get_yl() {
        return _yl;
    }

    public void set_yl(String _yl) {
        this._yl = _yl;
    }

    public String get_ypfl() {
        return _ypfl;
    }

    public void set_ypfl(String _ypfl) {
        this._ypfl = _ypfl;
    }

    public String getZlxl() {
        return zlxl;
    }

    public void setZlxl(String zlxl) {
        this.zlxl = zlxl;
    }

    public String getF21() {
        return f21;
    }

    public void setF21(String f21) {
        this.f21 = f21;
    }
}
